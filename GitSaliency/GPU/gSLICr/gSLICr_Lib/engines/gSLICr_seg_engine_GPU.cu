...
__global__ void Cvt_Img_Space_device__(const Vector4u* inimg, Vector4f* outimg, Vector2i img_size, COLOR_SPACE color_space, float *a, float *b, float *L);
...
{
	delete accum_map;
	// This delete was not in the original code and leads a memory leaks
	delete tmp_idx_img;
}
...
void gSLICr::engines::seg_engine_GPU::Cvt_Img_Space_(UChar4Image* inimg, Float4Image* outimg, COLOR_SPACE color_space, float *a, float *b, float *L)
{
	Vector4u* inimg_ptr = inimg->GetData(MEMORYDEVICE_CUDA);
	Vector4f* outimg_ptr = outimg->GetData(MEMORYDEVICE_CUDA);
	Vector2i img_size = inimg->noDims;
	...
	Cvt_Img_Space_device__ << <gridSize, blockSize >> >(inimg_ptr, outimg_ptr, img_size, color_space,a,b,L);
...
void gSLICr::engines::seg_engine_GPU::Find_Center_Association()
{
	...
	dim3 gridSize((int)ceil((float)img_size.x / (float)blockSize.x), (int)ceil((float)img_size.y / (float)blockSize.y));
	// EA
	//float invwt = 1.0f / ((spixel_size / gSLICr_settings.coh_weight)*(spixel_size / gSLICr_settings.coh_weight));
	...
	//Find_Center_Association_device << <gridSize, blockSize >> >(img_ptr, spixel_list, idx_ptr, map_size, img_size, spixel_size, invwt, max_xy_dist, max_color_dist);
}
...
// EA 
void gSLICr::engines::seg_engine_GPU::ComputePixelMap(){
	int sizeMatrix = idx_img->dataSize;
	int width = 3;
	float alpha = 0.9f;
	float sigma_2 = 0.05;
	float cp, lp, cp_, lp_, wpq, sum;
	idx_img->UpdateHostFromDevice();
	cvt_img->UpdateHostFromDevice();
	int* idx_img_ptr = idx_img->GetData(MEMORYDEVICE_CPU);
	float *w;
	spixel_info * pixelMap= spixel_map->GetData(MEMORYDEVICE_CPU);
	Vector4f * imgConverted= cvt_img->GetData(MEMORYDEVICE_CPU);
	printf("%d %d\n", spixel_map->noDims.x,spixel_map->noDims.y);
	int numSuperPixels=0;
	for (int i = 0; i < idx_img->dataSize; i++){
		if (idx_img_ptr[i]>numSuperPixels){
			numSuperPixels = idx_img_ptr[i];
		}
	}
	if (gSLICr_settings.color_space == gSLICr::CIELAB){
		printf("numSuperPixels %d L %f a %f b %f\n", numSuperPixels, imgConverted[0].x, imgConverted[0].y, imgConverted[0].z);
		
	}
	w = (float*)malloc(sizeof(float)*numSuperPixels*numSuperPixels);

	for (int i = 0; i < numSuperPixels; i++){
		for (int j = 0; j < sizeMatrix; j++){
			if (idx_img_ptr[j] == i){
				pixelMap[i].color_info.x += imgConverted[j].x;
				pixelMap[i].color_info.y += imgConverted[j].y;
				pixelMap[i].color_info.z += imgConverted[j].z;
				pixelMap[i].no_pixels++;
				
				pixelMap[i].center.height += (j / width) ;
				pixelMap[i].center.width += (j % width) ;
			}
		}
		pixelMap[i].color_info.x = pixelMap[i].color_info.x / pixelMap[i].no_pixels;
		pixelMap[i].color_info.y = pixelMap[i].color_info.y / pixelMap[i].no_pixels;
		pixelMap[i].color_info.z = pixelMap[i].color_info.z / pixelMap[i].no_pixels;
		pixelMap[i].center.height = pixelMap[i].center.height / pixelMap[i].no_pixels;
		pixelMap[i].center.width = pixelMap[i].center.width / pixelMap[i].no_pixels;
	...
	memset(w, 0, sizeof(float) * numSuperPixels*numSuperPixels);
	for (int j = 0; j < numSuperPixels; j++){
		for (int k = j + 1; k < numSuperPixels; k++){
			

			sum = alpha* (pixelMap[j].color_info.x - pixelMap[k].color_info.x)* alpha*(pixelMap[j].color_info.x - pixelMap[k].color_info.x)
				+ alpha* (pixelMap[j].color_info.y - pixelMap[k].color_info.y)*alpha*(pixelMap[j].color_info.y - pixelMap[k].color_info.y)
				+ alpha* (pixelMap[j].color_info.z - pixelMap[k].color_info.z)*alpha*(pixelMap[j].color_info.z - pixelMap[k].color_info.z)
				+ (pixelMap[j].center.height - pixelMap[k].center.height)*(pixelMap[j].center.height - pixelMap[k].center.height)
				+ (pixelMap[j].center.width - pixelMap[k].center.width)*(pixelMap[j].center.width - pixelMap[k].center.width);
			sum = (-1) *sum;
			wpq = exp(sum / (2 * sigma_2));
			w[j * 4 + k] = wpq;
			w[k * 4 + j] = wpq;

			printf("%i %i wpq %f sum %f\n", j, k, wpq);
