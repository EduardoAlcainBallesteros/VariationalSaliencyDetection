...
void gSLICr::engines::core_engine::ComputePixelMap(){
	slic_seg_engine->ComputePixelMap();
}


const IntImage* gSLICr::engines::core_engine::GetMask() const 
{
	const IntImage* idx_img = slic_seg_engine->Get_Seg_Mask();
	return idx_img;
}
...
void gSLICr::engines::core_engine::ConvertToArrays(float *dev_a, float *dev_b, float * dev_L,int size) 
{
	slic_seg_engine->ConvertToArrays(dev_a, dev_b, dev_L,size);
	float *a_ = (float *)malloc(sizeof(float) * 300);
	cudaMemcpy(a_, dev_a, 300 * sizeof(int), cudaMemcpyDeviceToHost);
}
